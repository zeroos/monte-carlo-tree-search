...
class Node:
    ...
    def get_preferred_child(self, root_node):
        ...
        # original library was choosing children by random. In order to make
        # the decision deterministic across multiple nodes we are just choosing
        # the first child.
    ...
    def get_score(self, root_node):
        discovery_operand = (
            ...
            sqrt(log(self.parent.visits) / (self.visits or 0.00001))
        )

        win_multiplier = (
            1
            if self.parent.player_number == root_node.player_number
            else -1
        )
    ...
    def is_scorable(self):
        return self.visits or self.policy_value is not None
